...
```yaml
---
title_page: "guap_lab"  # шаблон титульника из templates/title_pages/ (без .docx); по умолчанию guap_lab
...
Сводка ниже — только для ориентировки. **Перед аутлайном прочитай раздел своего типа работы в PROFILES.md** — там детали структуры, шаблон реферата ВКР, конвенции приложений и детальные правила лабораторного отчёта.

| Тип работы | Титульник | YAML-флаги | Ядро структуры |
|---|---|---|---|
| Лабораторная | `guap_lab` | — | цель → задание → ход работы → выводы |
| Курсовая | вузовский (`guap_course`, `spbgeu_course`, …) | `add_toc`, `number_headings`, `sections_new_page` | введение → 2–4 раздела → заключение → источники → приложения |
| ВКР | вузовский | как курсовая | как курсовая + **реферат первым разделом** |
| Практика | вузовский | как курсовая | основная часть по программе практики |
| Реферат | вузовский | `add_toc` при >10 стр. | введение → 2–3 раздела → заключение → источники |
| Контрольная | вузовский | — | задания как разделы |

Подписные документы (задание, дневник практики, отзывы) скилл не генерирует — пользователь вкладывает их сам.
...
- **pandoc**: диапазон 2.17–3.6 даёт идентичный результат (расхождения косметические); при странностях таблиц/листингов первым делом обнови pandoc до 3.x.
